...
import { FAQPageLd, MedicalProcedureLd, BreadcrumbListJsonLd } from '@/components/seo/JsonLd';
...
export default async function EndosPage({
  ...
  const [endosPageData, faqs, t, tNav] = await Promise.all([
  ...
  return (
    <>
      <HeroPreload name="endos" />
      <BreadcrumbListJsonLd
        locale={locale}
        items={[
          { href: ROUTES.home, name: tNav('home') },
          { href: ROUTES.endos, name: tNav('endos') },
        ]}
      />
    <div className="endos-page">
      ...
      <section className="bg-gray-50 py-20 md:py-28" aria-labelledby="endos-procedures-title">
        <div className="container-onkimia">
          ...
          {/* flex (no grid): this way an incomplete last row (5 items → 3+2)
              centers its cards instead of staying left-aligned under a
              fixed column track. Same pattern as MoreThanMedicine.tsx. */}
          <div className="flex flex-wrap justify-center gap-4 mb-4">
            {PROCEDURE_ITEM_KEYS.map((key, i) => {
              const Icon = PROCEDURE_ICONS[i];
              return (
                // items-center: the title is a single line (the description
                // below is commented out); with items-start the 44px icon
                // sits higher than the text's visual center.
